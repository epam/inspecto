...
export const OVERLAPPING_BONDS = "overlapping-atoms:6";

export const overlappingBondsAlgorithm: RuleAlgorithm<OverlappingBondsConfigType> = (structure, config) => {
    ...
    for (let i = 0; i < bonds.length; i++) {
      ...
      for (let j = i + 2; j < bonds.length; j++) {
        ...
        if (t1 >= 0 && t1 <= 1 && t2 >= 0 && t2 <= 1) {
          output.push({
            errorCode: OVERLAPPING_BONDS,
            isFixable: false,
            path: `${molecule.id}->bonds->${molecule.getBondIndex(bond)}`,
            message: "Bond has interaction with " + `${molecule.id}->bonds->${molecule.getBondIndex(nextBond)}`,
          });
        }
      }
    }

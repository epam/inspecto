...
export const getAngleBetweenBonds: GetAngleBetweenBonds = (bond1, bond2) => {
  ...
  throw new Error(
    "Bonds are not connected to each other directly or not conntecte at all",
  );
};
...
const getThridSideLength: GetThridSideLength = (bond1, bond2) => {
  if (bond1.to === bond2.to) {
    return getDistanceBetweenTwoAtoms(bond1.from, bond2.from);
  }

  if (bond1.to === bond2.from) {
    return getDistanceBetweenTwoAtoms(bond1.from, bond2.to);
  }

  if (bond1.from === bond2.to) {
    return getDistanceBetweenTwoAtoms(bond1.to, bond2.from);
  }

  if (bond1.from === bond2.from) {
    return getDistanceBetweenTwoAtoms(bond1.to, bond2.to);
  }
